 ...
 * 
 * We hook into DamageBlockEvent to detect when the block is about to be destroyed.
 * This prevents the vanilla block dropping logic (which ignores BreakBlockEvent cancellation
...
public class EnchantmentSilktouchSystem extends EntityEventSystem<EntityStore, DamageBlockEvent> {
    ...
    public void handle(int index,
        ...
        BlockHealthChunk healthChunk = chunkStore.getComponent(chunkRef, BlockHealthModule.get().getBlockHealthChunkComponentType());
        if (healthChunk == null) {
            return;
        }

        boolean willBreak = healthChunk.isBlockFragile(targetBlock) || (event.getCurrentDamage() - event.getDamage() <= 0.001f);
        if (!willBreak) {
            return;
        }
        ...
        // IMPORTANT: Cancel the DamageBlockEvent so vanilla drops (and block breaking logic) don't trigger
        ...
        // Apply Durability manually, since we cancelled DamageBlockEvent and bypassed BlockHarvestUtils calculation
        if (breakerRef != null && breakerRef.isValid()) {
            com.hypixel.hytale.server.core.entity.Entity rawEntity = com.hypixel.hytale.server.core.entity.EntityUtils.getEntity(breakerRef, store);
            if (rawEntity instanceof LivingEntity entity) {
                byte activeHotbarSlot = entity.getInventory().getActiveHotbarSlot();
                if (activeHotbarSlot != -1 && entity.canDecreaseItemStackDurability(breakerRef, store) && !tool.isUnbreakable()) {
                    ...
                    if (itemTool != null) {
                       com.hypixel.hytale.server.core.asset.type.item.config.ItemTool.DurabilityLossBlockTypes[] customLoss = itemTool.getDurabilityLossBlockTypes();
                       if (customLoss != null) {
                    ...
                    entity.updateItemStackDurability(breakerRef, tool, entity.getInventory().getHotbar(), activeHotbarSlot, -durabilityLoss, store);
                }
            }
        }
    }
