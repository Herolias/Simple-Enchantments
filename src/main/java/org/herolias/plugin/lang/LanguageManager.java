 ...
 * This allows forcing a specific language translation regardless of the
 * client's game language.
 */
public class LanguageManager {
     ...
     * Falls back to English, then finally returns the raw key if no translation
     * exists.
     */
    ...
    /**
     * Gets a localized Message object, resolving "default" via the client's
     * language.
    ...
    public Message getMessage(String key, String langCode, String clientLangCode) {
        ...
        // If we only got the key back, defer to client just in case the client has it
        // built-in
        if (raw.equals(key)) {
    ...
    /**
     * Retrieves the entire translation map for a specific target language,
     * including fallbacks.
     */
    ...
    /**
     * Pushes the entire translation map to the client using an
     * UpdateTranslations(Init) packet.
     * This forces the client to use these translations for all static UI, item
    ...
    public void sendUpdatePacket(com.hypixel.hytale.server.core.universe.PlayerRef playerRef, String customLangCode) {
        ...
        if (!translationsMap.isEmpty()) {
            ...
            // Use AddOrUpdate type to merge with the client's internal translation
            // dictionary
            com.hypixel.hytale.protocol.packets.assets.UpdateTranslations packet = new com.hypixel.hytale.protocol.packets.assets.UpdateTranslations(
